...
export function getFirstAndLastChar(order) {
  ...
  const first = order.charAt(0); // charAt doesn't support neg indexing and returns "" if not found
  const last = order.at(-1); // supports neg indexing and returns `undefined` if not found
  return {first, last};
}
