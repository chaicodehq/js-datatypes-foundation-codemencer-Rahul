...
export function writePostcard(sender, receiver, message) {
  if([sender,receiver,message].some(
...
export function isValidPincode(code) {
  ...
  if((!code.startsWith("0")) && (code.length === 6) && (code.match(/^\d+$/) !== null)) return true
  return false;
}

export function formatPostcardField(label, value, width = 12) {
  if([label, value].some((i) => typeof i !== 'string')) return "";
  return `${label.padEnd(width)}: ${value}`;
}

export function isFromState(address, stateCode) {
  if([address,stateCode].some(i => typeof i !== 'string')) return false;
  return address.endsWith(stateCode);
}

export function countVowels(message) {
  if(typeof message !== 'string') return 0;
  const vowelCnt = message.match(/[aeiouAEIOU]/g);
  if(vowelCnt === null) return 0;
  return vowelCnt.length;
}
